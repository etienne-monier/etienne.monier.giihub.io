...
if strcmp(action,'initialize'),
   ...
   btnNumber=3;
   yPos=menutop-(btnNumber-1)*(btnHt+spacing);
   labelStr='Ordre du filtre (attention : -1)';
   top = yPos - btnHt - spacing;
   labelWidth = frmWidth-.01;
...
% Bouton "Temps de propagation"
   ...
   labelList='Aucune|Gabarit|Courbe|Coefficients (RIF)';
   cmdList=str2mat( ...
	'1', ...    % Aucune
	'2', ...    % Gabarit du filtre
	'3', ...    % Courbe affichée
...
hndlList=[freqzHnd win1 win2 win3 methodHndl btn1Hndl btn2Hndl ord1Hndl ...
   ...
   % tmp, tmp2 = system('echo "rm filtnum.m" | at 18:00');
   return
...
elseif strcmp(action,'axesredraw'), % redraw axes
   ...
   if (~get(HOLDDD,'Value'))
	if ~isempty(autcourb)
       for ind = 2:length(autcourb)
            set(autcourb(ind),'visible','off');
       end
       autcourb = gobjects(0);
    end
    %hold off
...
elseif strcmp(action,'sauve'),
   ...
   elseif svg==4,
    [nom,chemin] = uiputfile('*.mat','Sauvegarde des coefficients RIF'); 
